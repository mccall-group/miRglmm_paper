...
fits=list()
start=Sys.time()
fits[["filter -1"]] = miRglmm(panel_B_filter, col_group=col_group_in, ncores = ncores, min_med_lcpm = -1)
end=Sys.time()
start=Sys.time()
fits[["filter -0.5"]] = miRglmm(panel_B_filter, col_group=col_group_in, ncores = ncores, min_med_lcpm = -0.5)
end=Sys.time()
fits[["filter 0"]] = miRglmm(panel_B_filter, col_group=col_group_in, ncores = ncores, min_med_lcpm = 0)
fits[["filter 0.5"]] = miRglmm(panel_B_filter, col_group=col_group_in, ncores = ncores, min_med_lcpm = 0.5)
fits[["filter 1"]] = miRglmm(panel_B_filter, col_group=col_group_in, ncores = ncores, min_med_lcpm = 1)
fits[["filter 1.5"]] = miRglmm(panel_B_filter, col_group=col_group_in, ncores = ncores, min_med_lcpm = 1.5)
fits[["filter 2"]] = miRglmm(panel_B_filter, col_group=col_group_in, ncores = ncores, min_med_lcpm = 2)

fits[["no filter"]] = miRglmm(panel_B_filter, col_group=col_group_in, ncores = ncores, min_med_lcpm = NULL)

# fit miRglmm poisson option 
fits[["filter -1 poisson"]] = miRglmm(panel_B_filter, col_group=col_group_in, family="poisson", ncores = ncores, min_med_lcpm = -1)
...
#run DESeq2 
panelB_raw=SummarizedExperiment(assays=list(t(miRNA_counts)), rowData=colnames(miRNA_counts), colData=cbind(col_group_in))
...
save(fits, file="ERCC/all_filters_results_noadjust.rda")
