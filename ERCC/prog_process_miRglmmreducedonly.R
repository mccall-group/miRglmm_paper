...
fits_in=fits[pull]
fits_in[["miRglmm"]]=fits_in[[filter_in]][["miRglmm_reduced"]] #substitute miRglmm reduced for miRglmm to be able to use existing processing code
...
#run LRT for random slope effect--dont need these for the miRglmm reduced only analysis
#LRTp=run_LRT(fits_in[["miRglmm"]], fits_in[["miRglmm_reduced"]])
#colnames(LRTp)=filter_in

#miRglmm variance components--dont need these for the miRglmm reduced only analysis 
#var_comp=list()
...
colnames(SE_hat)[1]=filter_in
  ...
  pull=c(filter_in, "miRglmnb", "DESeq2", "edgeR", "limvoom")
  fits_in=fits[pull]
  fits_in[["miRglmm"]]=fits_in[[filter_in]][["miRglmm_reduced"]]
  fits_in[["miRglmm_reduced"]]=fits_in[[filter_in]][["miRglmm_reduced"]]
  fits_in[[filter_in]]=NULL
...
save(results, file='ERCC/allfilters_processed_results_miRglmmreducedonly.rda')
