...
idx=which(comb_df$miRglmnb.y=="significant for miRglmm only" & comb_df$DESeq2.y=="significant for miRglmm only" & comb_df$edgeR.y=="significant for miRglmm only" & comb_df$limmavoom.y=="significant for miRglmm only")
...
write.csv(table_out, file="figures/SupTable7.csv", row.names=FALSE)