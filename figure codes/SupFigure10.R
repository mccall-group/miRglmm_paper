...
contrast_in="Natural Killer vs B lymphocyte"
...
sig_factor=factor(comb_df$limmavoom.y, levels=c("significant for aggregation method only", "significant for both", "significant for miRglmm only", "significant for neither"))
p4=ggplot(comb_df, aes(x=limmavoom.x, y=miRglmm, color=sig_factor))+geom_point(size=3)+geom_abline()+xlab('limma-voom logFC')+ylab('miRglmm logFC')+
  scale_colour_discrete(drop=FALSE, labels=levels(sig_factor), breaks=levels(sig_factor))+labs(color="Significance")+
...
tiff(file="figures/supfigure10B.tif", height=6.74, width=8.9, units="in", res=320)
print(myplot)
dev.off()
...
ggsave("figures/supfigure10.tif", plot=last_plot(), device="tiff", width=12.8, height=6.64, units="in", dpi=320, bg="white")
