...
p6=ggplot(comb_df, aes(x=Pool, y=estimate, group=sequence, color="miRglmm isomiR estimates (random effects)"))+geom_line(alpha=0.5)+geom_point(alpha=0.5)+
  geom_line(data=comb_df_overall, aes(x=Pool, y=estimate, group=1, color="miRglmm estimates (fixed effects)"), size=2)+theme_minimal()+
...
ggsave("figures/resub/figure5.tif", plot=last_plot(), device="tiff", width=16.8, height=9, units="in", dpi=320, bg="white")
