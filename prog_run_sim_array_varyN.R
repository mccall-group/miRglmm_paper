...
N=15
col_group_in = c(rep("A", N), rep("B",N))

source('miRglmm.R')
source('miRglm.R')

#fit miRglmm full and reduced models
fits = miRglmm(sims[[ind_run]]$sim_se[, c(1:N, 19+(1:N))], col_group=col_group_in, ncores = ncores)

#fit miRglmm full and reduced models
fits[["miRglmm poisson"]] = miRglmm(sims[[ind_run]]$sim_se[, c(1:N, 19+(1:N))], col_group=col_group_in, ncores = ncores, family="poisson")

#aggregate data to miRNAs and fit miRglmnb
miRNA_counts = t(apply(assay(sims[[ind_run]]$sim_se[, c(1:N, 19+(1:N))]), 2, function(x) by(x, rowData(sims[[ind_run]]$sim_se[, c(1:N, 19+(1:N))])$miRNA, sum)))
...
save(fits, file=paste0("sim results/sims_N100_m2_s1_rtruncnorm13_results", ind_run, "_N", N, ".rda"))
